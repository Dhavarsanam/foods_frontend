...
class _LoginScreenState extends State<LoginScreen>
  ...
  Future<void> _onGenerateOtp() async {
    ...
    final success = await ApiService.sendOtp(_mobileController.text.trim());
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(_snackBar(
        'OTP உங்கள் எண்ணிற்கு அனுப்பப்பட்டது',
      ));
    } else {
      ScaffoldMessenger.of(context).showSnackBar(_snackBar(
        'OTP அனுப்ப முடியவில்லை. Server running-ஆ இருக்கான்னு check பண்ணவும்.',
        isError: true,
      ));
    }
  }
  ...
  SnackBar _snackBar(String msg, {bool isError = false}) => SnackBar(
    content: Text(msg),
    backgroundColor:
    isError ? const Color(0xFFE53935) : const Color(0xFFE87722),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
              color: const Color(0xFFE87722).withOpacity(0.35),
              blurRadius: 18,
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          ...
          _inputField(
            controller: _hotelNameController, hint: 'Hotel Name',
            icon: Icons.storefront_rounded,
            validator: (v) => (v == null || v.trim().isEmpty)
                ? 'Hotel name உள்ளிடவும்' : null,
          ),
          const SizedBox(height: 14),
          _inputField(
            controller: _ownerNameController, hint: 'Owner Name',
            icon: Icons.person_rounded,
            validator: (v) => (v == null || v.trim().isEmpty)
                ? 'Owner name உள்ளிடவும்' : null,
          ),
          const SizedBox(height: 14),
          _inputField(
            controller: _mobileController, hint: 'Mobile Number',
            icon: Icons.phone_rounded,
            keyboardType: TextInputType.phone, maxLength: 10,
            formatters: [FilteringTextInputFormatter.digitsOnly],
            validator: (v) {
              if (v == null || v.trim().isEmpty)
                return 'Mobile number உள்ளிடவும்';
              if (v.trim().length != 10)
                return 'சரியான 10 இலக்க number உள்ளிடவும்';
              return null;
            },
          ),
          ...
          _gradientButton(
            ...
            onTap: _isLoading ? null : _onGenerateOtp,
          ),
        ],
      ),
    );
        ...
        children: [
          GestureDetector(
            ...
            child: Row(
              ...
              children: const [
                ...
                SizedBox(width: 4),
                Text('Change Details',
                    style: TextStyle(color: Color(0xFFE87722),
          ...
          const Text('Verify OTP 🔐',
              style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800,
                  color: Color(0xFF1A1A1A))),
          const SizedBox(height: 6),
          Text('+91 ${_mobileController.text} எண்ணிற்கு OTP அனுப்பப்பட்டது',
          ...
          _inputField(
            ...
            validator: (v) {
              ...
              if (v.trim().length != 6)
                return 'OTP 6 இலக்கங்கள் இருக்க வேண்டும்';
              return null;
            },
          ),
          const SizedBox(height: 14),
          Center(
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                const Text('OTP வரவில்லையா? ',
                    style: TextStyle(fontSize: 13, color: Color(0xFF999999))),
                GestureDetector(
                  onTap: _resendSeconds == 0 ? _onResendOtp : null,
                  child: Text(
                    _resendSeconds > 0
                        ? 'Resend in ${_resendSeconds}s' : 'Resend OTP',
                    style: TextStyle(
                      fontSize: 13, fontWeight: FontWeight.w700,
                      color: _resendSeconds > 0
                          ? const Color(0xFFBBBBBB) : const Color(0xFFE87722),
                    ),
                  ),
                ),
              ],
          ...
          const SizedBox(height: 28),
          _gradientButton(
            label: 'Verify & Login', icon: Icons.verified_rounded,
